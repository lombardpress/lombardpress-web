...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    ...
    <h3 class="lbp-side-bar-paragraph-heading"><a class="js-side-bar-scroll-to-paragraph" data-pid="{$pid}">Variants</a></h3>
    ...
    <li class="lbp-side-window-variant" data-lem-ref="lbp-app-lem-{$appnum}">
      ...
      <xsl:for-each select="tei:rdg">
        <xsl:choose>
          ...
          <xsl:when test="contains(@type, 'addCorrection') or ./@type='corrAddition' or ./@type='correction-addition'"> <!-- eventually the "contains" options should be removed as "corrDeletion, corrAddition" becomes the standard in the app schema -->
            <xsl:value-of select="."/><xsl:text> </xsl:text>
            <em>add.</em><xsl:text> </xsl:text>
            <xsl:value-of select="translate(@wit, '#', '')"/><xsl:text>   </xsl:text>
          </xsl:when>
          <xsl:when test="contains(@type, 'add.sed.del.') or ./@type='corrDeletion' or ./@type='correction-deletion'">
